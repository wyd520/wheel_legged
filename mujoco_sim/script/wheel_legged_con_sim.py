...
init_joint_pos = np.array([ 0, -0.07, 0, -0.07, 0.00, 0.913, -1.85, 0.00, 0.913, -1.85])
init_base_pos = np.array([0.0, 0, -0.0])
...
class WheelLeggedSim(MuJoCoBase):
  def __init__(self, xml_path):
    ...
    self.data.qpos[-10:] = init_joint_pos

    self.data.qvel[:3] = np.array([0, 0, 0])
    self.data.qvel[-10:] = np.zeros(10)
  ...
  def targetTorqueCallback(self, data):
    ...
    # print("targetTorque:", self.targetTorque)
  ...
  def targetVelCallback(self, data):
    ...
    # print("targetVel:", self.targetVel)

  def targetKpCallback(self, data):
    self.targetKp = data.data
    # print("targetKp:", self.targetKp)
    ...
    while not glfw.window_should_close(self.window):
      ...
      while (self.data.time - simstart <= 1.0/60.0 and not self.pause_flag):
        ...
        if (time.time() - sim_epoch_start >= 1.0 / self.sim_rate):
          # MIT control
          self.data.ctrl[:] = self.targetTorque + self.targetKp * (self.targetPos - self.data.qpos[-10:]) + self.targetKd * (self.targetVel - self.data.qvel[-10:])
          # print("ctrl:", self.data.ctrl[0:4])
          # print("pause_flag is 0")
          # Step simulation environment
          mj.mj_step(self.model, self.data)
          sim_epoch_start = time.time()
          # print("mass:", self.model.body_mass)
          # print("inertia:", self.model.body_inertia)
        ...
        if (self.data.time - publish_time >= 1.0 / 500.0):
          ...
          # print("jointsPosVel:", jointsPosVel.data)
      ...
      if (self.data.time - torque_publish_time >= 1.0 / 40.0):
        ...
        print("targetTorque:", targetTorque.data[0:4])
...
def main():
    ...
    xml_path = hector_desc_path + "/urdf/wheel_legged_con.xml"
